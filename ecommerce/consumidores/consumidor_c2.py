...
def main():
    ...
    canal.queue_declare(queue=FILA_C2, durable=True)

    canal.queue_bind(exchange=rmq.EXCHANGE_PROMOCAO, queue=FILA_C2, routing_key="promocao.categoria.*")

    canal.basic_consume(queue=FILA_C2, on_message_callback=receber_promocao, auto_ack=False)

    print("Consumidor C2 iniciado.")
    print("Interessado nas categorias A, B e C.")
